...
int main() {
  ...
  while (!gameOver)  // round start
  {
    ...
    if (strcmp(firstCommWord, "start") == 0) {
      ...
      if (hasGameStarted) {
        const char* PROMPT =
            "This will reset this whole game and start it from scratch. "
            "Continue?";
      ...
      revealTrump(deck, deckSize,
                  trumpSuit);  // Top card goes under and becomes a trump
      ...
      if (isMarriageDeclaredAndCardMustBePlayed) {
        ...
        if (strcmp(cardToPlay.suit, declaredMarriageSuit) != 0 ||
          ...
          std::cout << "You must play either the King or Queen of the declared "
                       "marriage suit (";
      ...
      if (!playSuccessful) {
        continue;  // Skip the rest of the play logic if the card wasn't played
                   // successfully
      }
      ...
      } else if (thrownCount == 2)              // If both players have played a card on the "table"
      {
        ...
        } else if (!isTrump(firstCard, trumpSuit) && isTrump(secondCard,
                                                             trumpSuit)) {             // only second card is a trump
        ...
        if (!isStockClosed) {
          ...
          if (deckSize == 0) {
            isStockClosed = true;
            std::cout << "Deck is empty. Stock closed. Strict rules are now in "
                         "effect."
                      << std::endl;
          }
        }
      ...
      if (P1HandSize == 0 && P2HandSize == 0) {
        ...
        if ((P1GamePoints >= requiredPointsToWin && P1GamePoints > preChangeP1GamePoints) ||
          ...
          gameOver = true;
          std::cout << "Game Over! Player " << (P1GamePoints >= requiredPointsToWin ? "1" : "2") << " wins the match!"
                    << std::endl;
          break;
        }
      ...
      for (int i = 0; i < (currentPlayerId == 1 ? P1HandSize : P2HandSize); i++) {
        ...
        if (strcmp(currentCard.suit, nine_Trump.suit) == 0 && strcmp(currentCard.rank, nine_Trump.rank) == 0) {
      ...
      if (isMarriageDeclaredAndCardMustBePlayed) {
        std::cout << "You have already declared a marriage and must play one "
                     "of its cards."
                  << std::endl;
        continue;
      }
      ...
      if (!P1hasWonCard && !P2hasWonCard) {
        std::cout << "You cannot declare a marriage during the first trick of "
                     "the round."
                  << std::endl;
        continue;
      }
